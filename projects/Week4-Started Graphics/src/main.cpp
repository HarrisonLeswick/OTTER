...
std::string windowTitle = "Harrison Leswick-100788569";
...
int main() {
	...
	//VBO - Vertex buffer object
	VertexBuffer::Sptr pos3_Vbo = VertexBuffer::Create();
	pos3_Vbo->LoadData(points3, 9);

	VertexBuffer::Sptr color2_vbo = VertexBuffer::Create();
	color2_vbo->LoadData(colors3, 9);

	VertexArrayObject::Sptr vao3 = VertexArrayObject::Create();
	vao3->AddVertexBuffer(pos3_Vbo, {
		BufferAttribute(0, 3, AttributeType::Float, 0, NULL)
		});
	vao3->AddVertexBuffer(color2_vbo, {
		{ 1, 3, AttributeType::Float, 0, NULL }
		});
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform4 = glm::translate(glm::mat4(1.0f), glm::vec3(glm::sin(static_cast<float>(thisFrame)),0.0f , 0.0f));
		...
		//Triangle 2
		shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection()* transform4 * transform3);
		vao3->Bind();
		glDrawArrays(GL_TRIANGLES, 0, 3);
		vao3->Unbind();
